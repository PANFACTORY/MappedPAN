...
function downloadmesh() {
    ...
    //.....節点のスイープ.....
    var pointids = new Array(nxi);
    var sweepedpointids = new Array(nxi);
    for(var i = 0; i < nxi; i++) {
        pointids[i] = new Array(nita);
        sweepedpointids[i] = new Array();
        for(var j = 0; j < nita; j++){
            pointids[i][j] = nita*i + j;
        }
    }
    for(var i = 0; i < nxi; i++) {
        for(var j = 0; j < nita; j++) {
            if(i == 0 || j == 0 || i == nxi - 1 || j == nita - 1) {
                for(var k = i; k < nxi; k++) {
                    for(var l = 0; l < nita; l++) {
                        if((k == 0 || l == 0 || k == nxi - 1 || l == nita - 1) && ((k == i && l > j) || (k != i))) {
                            if(Math.abs(meshs[i][j][0] - meshs[k][l][0]) < 1.0e-10 && Math.abs(meshs[i][j][1] - meshs[k][l][1]) < 1.0e-10) {
                                pointids[k][l] = pointids[i][j];
                                sweepedpointids[k].push(l);
                                for(var m = k; m < nxi; m++) {
                                    for(var n = 0; n < nita; n++){
                                        if((m == k && n > l) || m != k) {
                                            pointids[m][n] -= 1;
                                        }
                                    }
    ...
    var nsweeped = 0;
    for(var i = 0; i < nxi; i++){
         nsweeped += sweepedpointids[i].length; 
    }
    var pointdata = "\nPOINTS\t" + (nxi*nita - nsweeped) + "\tfloat\n";
    for (var i = 0; i < nxi; i++) {
        for (var j = 0; j < nita; j++) {
            if(sweepedpointids[i].indexOf(j) < 0){
